...
#' @name plot_event
#' @description This function takes a list of games and plots a heatmap of where a specific piece is captured on the board.
#' @param data The list of games to look for the event
#' @param event The name of the piece that is captured. "k" = checkmate, "q" = queen, "r" = rook, "b" = bishop, "n" = knight
#' @param color The color of the piece that is captured. "w" = white, "b" = black
#' @param position The of the piece when there are two for a color. For instance the left rook. "l" = left, "r" = right
#' @param transformation How should the counts of the heatmap be transformed. "none" = count, "sqrt" = count^.5, "log" = og(count + 1)
